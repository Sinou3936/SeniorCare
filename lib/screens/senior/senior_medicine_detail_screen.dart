            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(
                    'ë³µìš© ?œìž‘: ${medicine.startDate.year}??${medicine.startDate.month}??${medicine.startDate.day}??,
                    style: const TextStyle(fontSize: 16, color: Color(0xFF666666)),
                  ),
                  const SizedBox(height: 6),
                  Text(
                    medicine.endDate != null
                        ? 'ë³µìš© ì¢…ë£Œ: ${medicine.endDate!.year}??${medicine.endDate!.month}??${medicine.endDate!.day}??
                        : 'ë³µìš© ì¢…ë£Œ: ë¯¸ì„¤??(ê³„ì† ë³µìš©)',
                    style: const TextStyle(fontSize: 16, color: Color(0xFF666666)),
                  ),
                ],
              ),
  ...
  Future<void> _confirmDelete(BuildContext context) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('${medicine.name}??ë¥? ?? œ? ê¹Œ??',
