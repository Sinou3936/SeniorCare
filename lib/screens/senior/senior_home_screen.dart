...
class _SeniorHomeScreenState extends State<SeniorHomeScreen> {
  ...
  static const _slotOrder = ['?ÑÏπ®', '?êÏã¨', '?Ä??, 'Ï∑®Ïπ®'];
  static const _slotIcons = {
    '?ÑÏπ®': Icons.wb_sunny_outlined,
    '?êÏã¨': Icons.light_mode_outlined,
    '?Ä??: Icons.nights_stay_outlined,
    'Ï∑®Ïπ®': Icons.bedtime_outlined,
  };

  String _slotLabel(DateTime dt) {
    if (dt.hour < 10) return '?ÑÏπ®';
    if (dt.hour < 14) return '?êÏã¨';
    if (dt.hour < 20) return '?Ä??;
    return 'Ï∑®Ïπ®';
  }

  String _weekdayStr(int wd) => ['??, '??, '??, 'Î™?, 'Í∏?, '??, '??][wd - 1];
                    ...
                    child: Row(
                      ...
                      children: [
                        Column(
                          ...
                          children: [
                            Text(
                              '${now.month}??${now.day}??${_weekdayStr(now.weekday)}?îÏùº',
              ...
              Expanded(
                child: logs.isEmpty
                        ...
                        : ListView(
                            ...
                            children: [
                              Text(
                                '${_selectedDate.month}??${_selectedDate.day}??Î≥µÏö© ?ÑÌô©',
                                style: const TextStyle(
                                  fontSize: 22,
                                  fontWeight: FontWeight.bold,
