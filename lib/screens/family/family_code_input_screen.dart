...
class _FamilyCodeInputScreenState extends State<FamilyCodeInputScreen> {
  ...
  Future<void> _connect() async {
    ...
    if (seniorUid == 'expired') {
      setState(() {
        _isLoading = false;
        _errorMessage = 'ÎßåÎ£å??ÏΩîÎìú?àÏöî. Î∂ÄÎ™®ÎãòÍª???ÏΩîÎìúÎ•??îÏ≤≠?òÏÑ∏??';
        _controller.clear();
      });
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = 'ÏΩîÎìúÎ•?Ï∞æÏùÑ ???ÜÏñ¥?? ?§Ïãú ?ïÏù∏?¥Ï£º?∏Ïöî.';
        _controller.clear();
      });
      _focusNode.requestFocus();
    }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Î∂ÄÎ™®Îãò ?±Ïùò "??ÏΩîÎìú" ?îÎ©¥?êÏÑú\n6?êÎ¶¨ ÏΩîÎìúÎ•??ïÏù∏?òÏÑ∏??,
                  style: TextStyle(
                      color: Colors.white70, fontSize: 17, height: 1.5),
                ),
                const SizedBox(height: 48),

                // ?®Í≤®Ïß?TextField (?§Ï†ú ?ÖÎ†• Ï≤òÎ¶¨)
                Opacity(
                  opacity: 0,
                  child: SizedBox(
                    height: 1,
                ...
                const Center(
                  child: Text(
                    'ÏΩîÎìú??Î∂ÄÎ™®Îãò ??????ÏΩîÎìú ?êÏÑú ?ïÏù∏?????àÏñ¥??,
