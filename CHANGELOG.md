...
### 2026-07-11 — 복약 기간(duration) 기능 (v1.0.4)

- **복용 종료 방식 재설계** — 달력 종료일(`endDate`) → **복용 기간(`durationDays`, 일수)**. 각 슬롯이 지정한 일수만큼 발동하고, 등록 시점에 이미 지난 슬롯은 다음날로 롤오버 (예: 낮에 1일치 등록 → 오후·저녁 오늘 + 오전 내일). `null` = 계속(무한 복용)
  - 배경: 기존엔 알람이 `matchDateTimeComponents.time`으로 매일 무한 반복하며 종료일을 무시해, 종료된 약이 다음날에도 계속 울리던 버그
- **발생일 단일 로직** — `MedicineSchedule.isSlotActiveOn / firstOccurrenceDate / hasActiveOnOrAfter` 순수 함수를 알람 스케줄과 로그 생성이 공유 (단위 테스트 12개)
- **알람 = 7일 롤링 윈도우** — 무한 반복 폐기, (날짜, 슬롯)별 개별 스케줄. 앱 실행·부팅·복용완료 시 윈도우를 앞으로 재연장(무한약도 끊기지 않게). 스케줄한 알람 ID를 저장해 재등록 시 전부 취소 → 중복·고아 알람 방지
- **복용 기간 입력 UI** — 날짜 피커 → 프리셋 버튼(1일/3일/1주일/2주일/계속) + 직접입력 ± 스테퍼 (시니어용 큰 버튼, 공유 위젯 `DurationSelector`)
- **홈 표시는 무한대** — 7일 윈도우는 "알람 등록 범위"일 뿐, 홈은 접근 날짜마다 로그를 온디맨드 생성해 무한약을 미래 어느 날짜든 표시
- **업그레이드 유령 알람 방어** — 옛 버전이 무한 반복으로 걸어둔 알람(앱 데이터 삭제로도 안 지워지고 시스템 AlarmManager에 남음)을 업그레이드 시 1회성으로 취소 (프로덕션 신규 설치엔 무관)
- 실기기 검증: 각 슬롯 N회 발동·롤오버·중복 없음·화면 표시 정상 확인. 설계·계획 문서 `docs/superpowers/specs|plans/2026-07-11-medication-duration-*`
...
- **문의 폼 → 이메일 발송** — 보호자 설정에 문의 폼(카테고리: 오류/버그·기능 제안·사용 문의·기타 + 내용). 보내기 → Cloud Function `sendInquiry`(nodemailer + Gmail SMTP)로 이메일 발송. Firestore 저장 없음, 인증 필요, 시크릿(GMAIL_USER·GMAIL_APP_PASSWORD)으로 발송 계정 보호. 휴대폰 정보(기기/OS/앱버전)를 함께 수집(`device_info_plus`)
  - 메일 형식: 제목 `[약봄 문의]`, 본문 `카테고리 / 사용자 ID / 휴대폰 정보 / 내용` + "조치 부탁드립니다"
...
- **세로 모드 고정** — `SystemChrome.setPreferredOrientations([portraitUp])`로 가로 회전 시 레이아웃 깨짐 방지 (앱 전체·알람 화면 포함)
